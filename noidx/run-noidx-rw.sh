    ...
    for j in $(seq $repeat); do
      if [ $w -ge $((logger_per_node*2)) ]; then
        if [ $logger_per_node -gt 0 ]; then
          ...
          local c="numactl --interleave=$node_list $silo_d/test_nlog_noidx/silo.exe $opt -buffer_num $buffer_num -buffer_size $buffer_size -affinity $a"
        else
          local c="numactl --interleave=$node_list $silo_d/test_0log_noidx/silo.exe $opt -thread_num $((w*node_num))"
        fi
        echo $c | tee -a $b.log
        $c |& tee -a $b.log
        if [ -f latency.dat ]; then
          mv latency.dat $b/latency-l$logger_per_node-w$((w*node_num))-bn$buffer_num-bs$buffer_size-r$j.dat
        fi
        sleep 4
      fi
    done
